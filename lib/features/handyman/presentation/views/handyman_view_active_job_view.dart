...
class HandymanViewActiveJobView extends StatelessWidget {
  ...
  const HandymanViewActiveJobView({super.key, required this.requestId});
...
abstract class _ActiveJobBase<T extends StatefulWidget> extends State<T>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<HandymanCubit>().getJobDetails(requestId);
    });
  ...
  // ── Actions ──────────────────────────────────────────────
  void completeJob(bool isDark, AppLocalizations l10n, HandymanJobDetailsEntity job) {
    showAnimatedConfirmDialog(
      ...
      title: job.status == 'Accepted'
          ? 'ابدء الوظيفه'
          : l10n.jobCompleteTitle,
        message: job.status == 'Accepted'
            ? 'هل انت متاكد انك تريد ان تبدء الوظيفه الان؟'
            : l10n.jobCompleteConfirm,
      cancelLabel: l10n.cancel,
        confirmLabel: job.status == 'Accepted'
            ? 'ابدء'
            : l10n.jobCompleteBtn,      isDanger: false,
      confirmColor: accent,
        onConfirm: () {
          print(job.status);
          ...
          if (job.status == 'Accepted') {
            ...
            cubit.updateJobStatus(
              requestId,
              const UpdateJobStatusRequest(status: 4),
            );
          } else if (job.status == 'InProgress') {
            print('InProgress');
            cubit.updateJobStatus(
              requestId,
              const UpdateJobStatusRequest(status: 5),
            );
          }
        }
    );
          ...
          child: Column(
            ...
            children: [
              ...
              AppDetailRow(
                ...
                value:
                '${job.scheduledDate.day.toString().padLeft(2, '0')}/'
                    '${job.scheduledDate.month.toString().padLeft(2, '0')}/'
                    '${job.scheduledDate.year} - '
                    '${job.scheduledDate.hour.toString().padLeft(2, '0')}:'
                    '${job.scheduledDate.minute.toString().padLeft(2, '0')}',
              ),
              AppDetailRow(
  ...
  List<Widget> buildActionButtons(
    ...
    // Hide buttons if the job is already completed or rejected
    ...
    return [
      animatedSection(
        ...
        _ActionButton(
          ...
          onTap: () => completeJob(isDark, l10n, job),
        ),
      ),
      SizedBox(height: 10.h),
      // animatedSection(
...
class _ActiveJobMobileBodyState extends _ActiveJobBase<_ActiveJobMobileBody> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<HandymanCubit, HandymanState>(
          ...
          return Scaffold(
            ...
            bottomSheet: _AnimatedBottomBar(
              child: Column(
                ...
                children: buildActionButtons(isDark, l10n,job),
              ),
            ),
          );
        });
                        ...
                        Expanded(
                          ...
                          child: animatedSection(
                            ...
                            _QuickActionsCard(
                              ...
                              l10n: l10n,
                              accent: accent,
                              onComplete: () => completeJob(isDark, l10n,job!),
