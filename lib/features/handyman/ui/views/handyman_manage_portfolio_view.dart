...
final _seedItems = [
  const PortfolioItem(
    ...
    description:
        'تم فك السخان القديم وتركيب سخان جديد مع تغيير المحابس والوصلات. العمل تم في ساعة واحدة.',
  ),
  const PortfolioItem(
    id: '2',
    ...
    description:
        'تغيير جميع مواسير الصرف والتغذية واختبار الضغط للتأكد من سلامة التوصيلات قبل التبليط.',
  ),
  const PortfolioItem(
    id: '3',
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (_, constraints) => constraints.maxWidth >= 600
          ? _PortfolioTabletBody()
          : _PortfolioMobileBody(),
    );
  }
}
...
abstract class _PortfolioBase<T extends StatefulWidget> extends State<T>
  ...
  final items = List<PortfolioItem>.from(_seedItems);
  ...
  void showAddSheet() {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _AddPortfolioSheet(accentColor: accent, onAdd: addItem),
    );
  }
                                ...
                                ...left.asMap().entries.map(
                                  (e) => Padding(
                                    ...
                                    child: _TabletPortfolioCard(
                                      ...
                                      index: e.key * 2,
                                      accent: accent,
                                      onDelete: () => confirmDelete(e.key * 2),
                                    ),
                                  ),
                                ),
                              ],
...
// ══════════════════════════════════════════════════════════════
// _AddPortfolioSheet — bottom sheet form
// ══════════════════════════════════════════════════════════════
...
class _AddPortfolioSheetState extends State<_AddPortfolioSheet>
  ...
  void _submit() {
    ...
    final item = PortfolioItem(
      id: '${now.millisecondsSinceEpoch}_${Random().nextInt(9999)}',
      category: _category!,
